...
셋업 스크립트가 수행하는 작업:
1. GitHub API로 최신 릴리즈 자동 감지 및 다운로드
2. `%USERPROFILE%\ExcelMcp\`에 mcp-excel.exe 추출 (zip 내부 구조 자동 탐색)
3. Windows PATH 환경변수에 등록 → `mcp-excel` 명령으로 실행 가능
4. 플러그인 `.mcp.json`에 실행 경로 기입
5. Claude Code / Claude Desktop / VS Code 설정 파일에 MCP 서버 자동 등록
6. stdio JSON-RPC 핸드셰이크로 실제 동작 검증

## 사전 조건

- **Windows** 환경 (Excel COM 자동화 필수)
- **Microsoft Excel 2016+** 설치
- 인터넷 연결 (GitHub에서 릴리즈 다운로드)
- setup 스크립트 실행 완료 후 `mcp-excel` 명령이 PATH에서 동작해야 함
