...
2. email-mcp 위치 확인 (`-EmailConnectorPath` 로 override 가능, 기본은 sibling)
3. 누락된 email-mcp 의존성 `pip install`
...
설치 후 **`<email-mcp>\.env`** 만 실제 값으로 편집하고 Claude Desktop 재시작.
...
## 일반 원칙
1. **email-mcp 셋업이 선행**되어야 한다. `email-mcp/SETUP.md`의 STEP A(진단)부터 통과한 상태가 출발점.
2. 모든 Python 호출은 `py -3.9` 명시 — `python` / `py` 단독은 다른 인터프리터를 잡을 수 있다.
3. WSL/macOS/Linux 감지 시 즉시 중단. 이 스킬은 Windows 네이티브 전용.

---

## STEP A. email-mcp 선행 검증 [AGENT][CHECK]

email-mcp는 email-mcp를 in-process import 하므로, 그쪽이 깨져 있으면 어느 도구도 동작하지 않는다.
...
- `all_ok: true` → STEP 1로.
- `all_ok: false` → `email-mcp/SETUP.md`의 STEP A 매핑 표대로 그쪽을 먼저 고친다. 여기서 더 진행 금지.

## STEP 1. email-mcp 폴더 배치 [USER + AGENT][CHECK]

email-mcp 폴더를 email-mcp와 **같은 부모 폴더** 아래에 두면 sibling 자동 탐지로 동작한다 (별도 환경변수 불필요).
...
dir "%USERPROFILE%\.claude\skills\email-mcp\scripts\search.py"
```
...
stderr에는 `[email-mcp] starting ... (email-mcp at ...)` 한 줄이 보일 수 있다 (정상).

**실패 시:**
- `email-mcp requires Python 3.9` → STEP 0 (Python 3.9 설치, email-mcp SETUP.md STEP 2 참조).
- `cannot find email-mcp at ...` → STEP 1 폴더 위치 확인 또는 `set EMAIL_MCP_PATH=...` 명시.
- `import scripts.config` 실패 → email-mcp 의존성 미설치. STEP A로 회귀.
...
`total_mails == 0` 이면 인덱싱이 안 된 상태 — `email-mcp` CLI 로 `py -3.9 scripts\ingest.py` 돌리거나, MCP `ingest {limit: 50}` 으로 일부만 부어 보고 다시 stats 확인.
