...
function Test-ServerImport {
    ...
    $out = & py -3 -c $script 2>&1 | Out-String
...
# 2. Python (>= 3.10 recommended; py -3 must resolve)
Write-Step "2. Python (py -3)"
try {
...
if ($SkipDeps) {
    ...
    if ($depCheck -match "^ok\s*$") {
        ...
        if ($DryRun) {
            Write-Dry "would run: py -3 -m pip install -r `"$reqPath`""
        } else {
