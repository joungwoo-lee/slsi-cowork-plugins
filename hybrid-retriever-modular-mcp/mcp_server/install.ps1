...
function Test-ServerImport {
    ...
    $out = & py -3.11 -c $script 2>&1 | Out-String
...
# 2. Python 3.11
Write-Step "2. Python (py -3.11)"
try {
    $pyVersion = & py -3.11 --version 2>&1
} catch [System.Management.Automation.CommandNotFoundException] {
    Fail "py launcher not found. Install Python 3.11 from python.org."
} catch {
    Fail "py launcher error: $_"
}
if ($LASTEXITCODE -ne 0) {
    Fail "py -3.11 failed: $pyVersion"
}
Write-Ok "$pyVersion"
...
if ($SkipDeps) {
    ...
    $depCheck = & py -3.11 -c "import dotenv,requests,qdrant_client,pypdf,docx,openpyxl,chardet,urllib3; print('ok')" 2>&1
    if ($depCheck -match "^ok\s*$") {
        Write-Ok "all dependencies importable"
    } else {
        Write-Warn "some dependencies missing - running pip install"
        if ($DryRun) {
            Write-Dry "would run: py -3.11 -m pip install -r `"$reqPath`""
        } else {
...
claude mcp add retriever_mcp "py -3.11 `"$serverPath`""
if errorlevel 1 (
    echo.
