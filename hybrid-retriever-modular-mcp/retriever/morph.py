...
# Characters with special meaning to the FTS5 query parser. We strip them so
# that arbitrary user input cannot blow up `MATCH ?` with a syntax error.
...
def tokenize_for_query(query: str) -> str:
    ...
    and phrases are space-separated (implicit AND). This makes the search
    behave like "find documents containing all of these morphemes" while
    being safe against FTS5 query-parser injection.
    """
    if not query:
        return ""
