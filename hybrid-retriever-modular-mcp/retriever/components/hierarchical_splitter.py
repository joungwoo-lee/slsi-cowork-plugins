"""Chunking component: flat or parent-child hierarchical, preserving legacy IDs.

Emits ``Document`` objects whose ``id`` matches the canonical chunk_id format
``{document_id}:{position}`` and whose ``meta`` carries every field the rest of
the pipeline (writer, retrievers, search response formatter) expects.

``use_hierarchical`` accepted forms:
  - ``"false"`` / ``False`` / ``None``  -> flat chunking (chunk_chars / overlap)
  - ``"true"``  / ``True``              -> parent-child (parent_*/child_* sizes)
  - ``"full"``                          -> single parent covering full text,
                                          children chunked from it
"""
...
class HierarchicalSplitter:
    ...
    def run(
        ...
        records = _make_records(text, self._cfg, use_hierarchical)
        if not records:
        ...
        for pos, rec in enumerate(records):
            ...
            documents.append(Document(id=chunk_id, content=rec["child_content"], meta=doc_meta))
        parent_count = len({r["parent_id"] for r in records if r["is_hierarchical"]})
        return {
            "documents": documents,
...
def _hierarchical_records(text: str, cfg: dict[str, int], parent_mode: str = "normal") -> list[dict]:
    if parent_mode == "full":
        ...
        parents = [
            (pid, p, _chunk_text(p, cfg["child_chunk_chars"], cfg["child_chunk_overlap"]))
            for pid, p in enumerate(parent_chunks)
        ]
    records: list[dict] = []
