...
> **한국어 검색**: FTS5는 `unicode61` 토크나이저를 쓰지만, 색인·쿼리 양쪽에서 `kiwipiepy` 형태소 분석을 거쳐 들어갑니다. "메일" / "엔진" 같은 **2글자 한국어**도 매치됩니다. 다만 합성어(예: "회의록")는 한 형태소로 취급되어 "회의"로는 잡히지 않습니다. 기존 DB는 첫 실행 시 자동으로 재색인됩니다 (`PRAGMA user_version=2`).
>
> **그래프 검색**: 임베디드 Kùzu(`GraphDB/` 디렉토리, 도커 불필요)로 Document/Chunk/Dataset 노드 + IN_DATASET/HAS_CHUNK/NEXT 엣지를 저장. `graph_rebuild` 한 번 호출 후 `graph_query`로 Cypher 사용.
...
| 기능 | 상태 |
|---|---|
| SQLite FTS5 keyword backend (kiwipiepy 형태소, 한국어 2글자 매치) | 지원 |
| Kùzu embedded graph (Cypher, no Docker) | `graph_query`, `graph_rebuild` |
