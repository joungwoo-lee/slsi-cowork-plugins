...
대상 폴더를 직접 스캔해 읽을 문서 목록을 만든다.

- 포함 대상: 사람이 읽을 수 있는 문서/텍스트 파일
- 제외 대상: `AGENTS.md`, `_closets/`, 숨김 디렉터리, 빌드 산출물, 의존성 디렉터리, 바이너리 파일
- 원본 폴더 구조는 그대로 유지하고, 원본 파일은 절대 수정하지 말 것

이미 인제스트된 폴더라도 기존 산출물을 기계적으로 신뢰하지 말고, 현재 폴더 상태를 기준으로 다시 판단한다.

### Step 2. 구조 판단

각 문서를 처음부터 끝까지 직접 읽고 아래를 판단한다.

- `wing`: 문서가 속한 상위 주제 묶음
- `room`: 문서를 대표하는 안정적인 room slug
- `dominant_hall`: `technical`, `decisions`, `problems`, `milestones`, `reference` 중 문서의 중심 성격

중요:

- 경로나 파일명만으로 기계적으로 분류하지 말 것
- 문서 내용이 경로명보다 우선한다
- 비슷한 주제의 문서는 같은 Room으로 합칠 수 있고, 하나의 문서라도 주제가 분리되면 별도 Room으로 나눌 수 있다
- 애매하면 더 보수적으로 분류하고, Room을 불필요하게 많이 쪼개지 말 것

### Step 3. Closet 파일 생성 (_closets/<room>.aaak.md)

판단한 Room 구조에 따라 `_closets/<room>.aaak.md`를 생성한다.
같은 `room`에 여러 파일이 있으면 하나의 Closet 파일에 합산한다.

Closet 파일을 쓰기 전에 반드시 `AAAK_WRITING_GUIDE.md`를 읽고 아래 형식과 필드 규칙을 검증한다.

중요: Closet 안의 AAAK는 얇은 문서 요약이 아니라 문서 내용을 AAAK로 가능한 충실하게 다시 적은 것이어야 한다. 문서 보조 메모나 중간 분류 결과에 의존하지 말고, 각 문서를 처음부터 끝까지 읽은 뒤 문서의 의미 있는 내용을 빠짐없이 zettel들로 풀어 써야 한다.
...
## SOURCE LINKS
- [jwt-design.md](../auth/jwt-design.md)
```
...
Tunnels는 문서 내용, 주제 중복, 상호 참조, 같은 개념 축을 근거로 직접 판단한다.
...
Wing/Room 구조는 현재 폴더의 원본 문서와 최신 Closet 내용을 기준으로 전체 재생성한다.
...
모든 Closet 파일과 `AGENTS.md`를 저장한 뒤 아래를 확인한다.

- `AGENTS.md`는 반드시 대상 폴더 루트에만 있는가
- Closet 파일은 반드시 `_closets/` 아래에만 있는가
- `SOURCE LINKS`가 실제 원본 파일 경로를 가리키는가
- Wing/Hall/Room/TUNNELS 구조가 현재 문서 집합과 모순되지 않는가
